...
function setupThemeToggle() {
    ...
    if (manualDarkMode === null) {
        ...
        shouldBeDark = manualDarkMode === 'true';
    }
    ...
    if (shouldBeDark) {
        ...
        if (mobileThemeToggle) mobileThemeToggle.innerHTML = '<i class="fa fa-sun-o"></i>';
    }
    ...
    // 设置定时器定期检查时间，以便在时间切换点自动更新主题
    setInterval(() => {
        // 只有在没有手动设置的情况下才根据时间自动切换
        if (localStorage.getItem('darkMode') === null) {
            const shouldBeDarkNow = isNightTime();
            const isDarkNow = document.body.classList.contains('dark-mode');
            
            if (shouldBeDarkNow !== isDarkNow) {
        ...
        setTimeout(() => {
            ...
            const books = [
                ...
                },
                {
                    ...
                    category: 'fiction',
                    coverPath: '',
                    description: '中国古典四大名著之一，描写了贾、史、王、薛四大家族的兴衰，以及贾宝玉、林黛玉、薛宝钗等人的爱情悲剧。',
                    filePath: '/book/红楼梦/第一回.md',
                    rating: 4.9,
                    reviewCount: 2345
                },
                {
                ...
                },
                {
                    ...
                    coverPath: '',
                    description: '从智人到智神，人类是如何一步步走到今天的？这本书将带你回顾人类历史的重大转折点。',
                    filePath: '/book/人类简史/引言.md',
                    rating: 4.6,
                    reviewCount: 3245
                }
            ];
...
function openBookReader(filePath, bookTitle) {
    // 检查filePath是否已经是HTML文件
    if (!filePath.endsWith('.html')) {
        // 如果是.md文件路径，尝试转换为对应的HTML文件路径
        // 例如：/book/乔布斯传/序 前言 本书是如何诞生的.md -> /乔布斯传.html
        const bookName = bookTitle || filePath.split('/').filter(Boolean)[1];
        if (bookName) {
            // 跳转到对应的HTML文件
            window.location.href = `/${bookName}.html`;
        } else {
            // 如果无法解析书名，显示提示
            alert(`请先创建${bookTitle || '该书籍'}的HTML内容文件，然后再尝试阅读。\n\n请参考BookTemplate.html和BookTemplate_Usage.md文件了解如何创建书籍HTML文件。`);
        }
    } else {
        // 已经是HTML文件，直接跳转
